  ...
  async function handleDownload(format) {
    ...
    try {
      if (format === 'txt') {
        ...
        // Bake computed CSS variable colors into inline styles so html2canvas can render them
        const snapEls = [el, ...el.querySelectorAll('*')];
        const snapOverrides = snapEls.map(e => {
          const cs = window.getComputedStyle(e);
          const orig = { color: e.style.color, backgroundColor: e.style.backgroundColor, borderColor: e.style.borderColor };
          e.style.color = cs.color;
          e.style.backgroundColor = cs.backgroundColor;
          e.style.borderColor = cs.borderColor;
          return { e, orig };
        });
        const canvas = await html2canvas(el, { scale: 2, useCORS: true, allowTaint: true, backgroundColor: '#0f0f1a' });
        snapOverrides.forEach(({ e, orig }) => { e.style.color = orig.color; e.style.backgroundColor = orig.backgroundColor; e.style.borderColor = orig.borderColor; });
        if (format === 'png') {
          const a = document.createElement('a');
