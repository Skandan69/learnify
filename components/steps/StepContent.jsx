...
export default function StepContent({ content, fmt, onContentChange, onFmtChange, onBack, onNext }) {
  ...
  async function handleImageUpload(e) {
    ...
    if (!file) return;
    setUploading(true);
    setUploadMsg('Reading your notes...');
    try {
  ...
  async function handleVideoUpload(e) {
    ...
    if (!file) return;
    setUploading(true);
    setUploadMsg('Extracting frames from video...');
    try {
    ...
    <div>
      ...
      {uploadMsg && (
        <p style={{
          ...
          color: uploadMsg.startsWith('Notes') || uploadMsg.startsWith('Video') ? 'var(--success-text)' : uploadMsg.startsWith('Error') ? 'var(--danger-text)' : 'var(--text2)',
        }}>
